...
public class MemberService {
	...
	public int deleteMember(Member member) {
		...
		// 커넥션 생성
		Connection conn = getConnection();
		// 멤버dao의 deletemember 메소드 반환값 저장 : 성공한 행의 개수 의미
		int result = dao.deleteMember(conn,member);
		// 성공하면 commit 실패시 rollback
		if(result>0) commit(conn);
		else		 rollback(conn);
		// 커넥션 반환
		close(conn);
	 ...
	 * @return
	 */
	public int updatePw(Member member, Member member2) {
		
		Connection conn = getConnection();
		
		// 멤버dao의 deletemember 메소드 반환값 저장 : 성공한 행의 개수 의미
		int result = dao.updatePw(conn,member,member2);
