...
check1.addEventListener("click", () => {
  ...
  //                            없으면 null
  console.log(regEx.exec(str1)); // ['123']
...
check2.addEventListener("click", () => {
  ...
  */
  ...
  regEx = /^[a-z]TEST\d$/;
  // 영어 소문자로 시작 + TEST + 끝은 1자리 숫자로 끝나는 문자열
  str = "wTEST4"
  console.log(`${str} : ${regEx.test(str)}`); // true
...
inputName.addEventListener("keyup", ()=>{
  ...
  inputName.value = value; // 좌우 공백이 제거된 값을 대입! 오 제거된당  
...
inputNumber.addEventListener("keyup",()=>{
  ...
  if(value.length == 0){
    numberResult.innerHTML = "휴대폰 번호 입력(- 포함)";
