...
// 함수도 변수에 대입 가능!!!
// -> 이 때 변수명이 함수명이 된다
// 요즘 트렌드 : 선언한 거보다 앞에 호출하면 오류
const test = function(){alert(123)};

// JS 코드 해석 순서
// 1. 현재 HTML에 연결된 모든 JS에서 선언된 함수를 먼저 인식 T
// 2. 위 -> 아래 차례대로 해석

/* 변수 = 함수명 형식으로 함수를 선언하는 경우 
    - 함수를 선언한 걸로 인식하는 것이 아닌 변수를 선언한 걸로 인식되어
    해석 순서 1번(함수 위치 인식)으로 인식되지 않아
    함수 호출 코드가 선언코드보다 앞에 작성되면 오류 발생(호출 불가)
*/
...
// 선언 + 정의 + 호출 한번에 수행해서 일반 함수보다 상대적으로 빠름
(function(){
...
arrowList[2].addEventListener("click", () => {
  // 다른 함수 호출 
  // 매개 변수로 a, b 두 값을 전달 받아 곱한 값을 
  // 반환하는 익명함수를 전달 (익명함수가 매개변수!)
...
// return 값이 object 타입이면 {return } 생략 불가
