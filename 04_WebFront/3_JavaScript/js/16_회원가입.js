...
inputId.addEventListener("input", e => {
  ...
  inputId.value = val; // 공백이 제거된 값을 input값으로 대입
  ...
  if(val.length === 0){ // 입력된 값이 없을 경우
    ...
    //유효성 검사 여부 저장 객체에서
    // " input-id"의 값을 false로 변경(유효하지 않다를 ㄸㅅ함)
...
inputPw.addEventListener("input", e => {
  ...
  const span = e.target.nextElementSibling.nextElementSibling;
  inputPw.value = val; // 공백이 제거된 값을 input값으로 대입
...
inputName.addEventListener("input", e => {
  ...
  const span = e.target.nextElementSibling.nextElementSibling;
  inputName.value = val; // 공백이 제거된 값을 input값으로 대입
...
signupForm.addEventListener('submit', e => {
  ...
  */
  ...
  for(let key in checkList){
    ...
    if(!checkList[key]){ // 유효하지 않은 값이 입력이 있을 경우
      let str;
      switch(key){
