...
// querySelector() -> 여러 요소가 선택돼도 첫 번째 요소 하나만 얻어옴
...
addBtn.addEventListener("click", () => {
  ...
  // 요소에 class 추가하기 -> 클래스만 classList 나머지는 setAttribute
  ...
  // 5) div.row를 .containter의 마지막 자식으로 추가
...
/* 
  ...
  3) for문으로 모든 요소 접근하여 작성된 값(value)를 얻어와
...
calcBtn.addEventListener("click",() => {
  const inputnums = document.getElementsByClassName("input-number");
  let sum = 0;

  for(let i =0; i<inputnums.length; i++){
    sum += Number(inputnums[i].value);
  }
