...
public class MemberController {
	...
	// Spring에서 파라미터를 전달 받는 방법1
	// - HttpServletRequest 이용
//	@RequestMapping(value="/login" , method = RequestMethod.POST)
	public String login(HttpServletRequest req) {
	
		/* - Argument Resolver(매개 변수 확인자(해결사))
		 * 매개변수에 원하는 타입을 선언하면 해당 타입 객체를 얻어오거나 생성할 수 있다.
...
//	@PostMapping("/login") // @RequestMapping의 POST 버전
	...
	// Spring에서 파라미터를 전달 받는 방법3 (이걸 많이 씀!)
...
//	@PostMapping("/login")
	...
	public String login(Member inputMember, Model model, RedirectAttributes ra) {
		...
		// - redirect 시 : session scope 이동
		// - redirect 완료 후 : requset scope로 돌아옴
	...
	public String signup(Member inputMember,
		...
		// 회원 가입 서비스 호출 후 결과(성공한 INSERT 행의 개수) 반환 받기
    ...
    * */
	...
	// @ExceptionalHandler(Exception.class)
