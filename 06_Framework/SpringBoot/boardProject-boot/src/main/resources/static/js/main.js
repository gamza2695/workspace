...
const getCookie = (key) => {
  ...
  // 현재 존재하는 쿠키를 모두 얻어옴
  const cookies = document.cookie; // 'saveId=member06@naver.com'

  // 배열.map(() =>{}) : 배열내 요소를 하나씩 순차접근 후
  // 함수 수행 후 반환되는 결과를
  // 새로운 배열에 담아서 반환

  // ['testCookie=테스트', 'saveId=member06@naver.com']
...
if(loginFrm!=null){
  ...
  loginFrm.addEventListener("submit", e=> {
    ...
    if(memberEmail.value.trim().length == 0){
      ...
      // submint 이벤트 발생시 form 태그의 기본 이벤트(제출) 제거
      e.preventDefault();
      return;
