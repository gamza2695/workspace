...
// RESTFUL API : REST가 매우 잘 적용된 API
...
// 등록, 수정, 삭제 버튼 누를 때마다 댓글 목록 다시 다 조회
...
const selectCommentList = () => {
    ...
    .then(response => response.json()) // 응답 객체 -> 파싱
                      // 백엔드에서 List로 받아옴 -> JSON으로 바꿔서 응답(바꾼것 clist)
    .then(cList => { // cList : 댓글 목록(객체 배열)
        console.log(cList);

        // 화면에 출력되어 있는 댓글 목록 삭제 (미리 다지움!)
...
addComment.addEventListener("click", e => { // 댓글 등록 버튼이 클릭이 되었을 때
    ...
    fetch("/comment",{
        ...
        body : JSON.stringify(data) // JS객체 -> JSON 파싱( "" 문자열로 바꾸는것!)
    })
    .then(resp => resp.text()) // result 단순 텍스트로 텍스트로 파싱
    .then(result => {
        if(result > 0){ // 등록 성공
            alert("댓글이 등록되었습니다.");
