...
// - 파일이 선택 화면에서 취소를 누르면 기존에 세팅된 value/files가 없어지게 된다!
...
// 프로필 이미지가 
// -1 : 변경되지 않았을 때 (프로필 이미지 변경 없을 시 변경못하게) 
//  0 : 있었는데 없어짐 == x 버튼 클릭
//  1 : 새 이미지가 선택 (없음 -> 있음, 있음 -> 다른 이미지)
let statusCheck = -1;

// input type = "file" 태그의 값이 변경되었을 때 변경된 상태를 백업해둘 변수
// 요소.cloneNode(true/false) : 요소 복제 (깊은 복사)(true이면 하위 요소도 복제)
let backupInput;
...
if(imageInput != null){ // #imageInput 존재할 때
    ...
    const changeImageFn = e =>{
        ...
        if(uploadFile == undefined){ // 취소를 눌러서 파일이 files[0]에 파일이 없을 때
        ...
        reader.onload = e => {
            ...
            // img 태그의 src 속성의 속성값으로 읽은 파일의 URL을 대입
