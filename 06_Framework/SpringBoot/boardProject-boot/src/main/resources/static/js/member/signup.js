...
sendAuthKeyBtn.addEventListener("click", function(){
  ...
  if(checkObj.memberEmail){ // 중복이 아닌 이메일인 경우에만 발송
...
memberNickname.addEventListener("input", () => {
  ...
  if(regEx.test(memberNickname.value)){
    ...
    fetch("/member/checkNick?nickname=" + memberNickname.value)
    .then(response => response.text())
    .then(result =>{ 
      if(result==0){ // 중복X
        ...
        nickMessage.classList.remove("confirm"); // 빨간 글씨 제거
    ...
    nickMessage.innerText = "알맞은 형식의 닉네임을 작성해주세요.";
...
memberTel.addEventListener("input", () => {
  ...
  if(regEx.test(memberTel.value)){
    ...
    telMessage.innerText = "알맞은 형식의 비밀번호를 작성해주세요.";
...
document.getElementById("signUpFrm").addEventListener("submit", e => {
  ...
  /* checkObj의 모든 값을 검사해서 하나라도 false이면 가입시도X */
