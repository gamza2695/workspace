...
selectMemberNo.addEventListener("click",() => {
  ...
  // => 요청 주소는 쿼리스트링 전까지 /ajax/selectMemberNo

  // 요청에 대한 응답이 돌아왔을 때 수행
  .then((response) => { // response : 응답이 담겨져있는 객체(promise 타입)
...
btn5.addEventListener("click", () => {
  ...
  .then(response => {
    // response == 응답 정보가 담긴 객체(상태코드, 데이터 등 들어있음)
    ...
    for(let member of memberList){ // 향상된 for문
      ...
      btn.addEventListener("click", e =>{
        ...
        fetch("/ajax/updateFlag", {
          ...
          body : JSON.stringify(obj) // JSON.stringify({"flag":flag, "targetNo":targetNo})
        })
...
btn6.addEventListener("click", () => {
  ...
  console.log(member); // JS 객체에 key/value 추가한거 확인
