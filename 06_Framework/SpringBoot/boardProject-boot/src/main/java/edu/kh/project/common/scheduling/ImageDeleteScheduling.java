...
//	@Scheduled(fixedRate = 5000) // 이전 작업이 시작 후 5초 후에 실행
	
	//cron="초 분 시 일 월 요일 [년도]" - 요일 : 1(SUN) ~ 7(SAT) 
//	@Scheduled(cron = "0,15,30,45 * * * * *") // 매달 1일 자정마다 수행
...
//	@Scheduled(cron = "0 0 0 1 * *") // 매달 1일 자정마다 수행
	public void scheduling(/*매개변수 존재하면 x!!*/) {
		
		log.info("스케쥴러 동작");
		
		// 지금까지 파일 업로드(프로필, 게시글) 
		// -> 이미지 수정하는 경우 DB만 변경되고 서버에는 이전 파일이 계속 남음
		// -> 불필요한 파일 정리하는 스케쥴러 동작 필요하다
...
//		log.debug(serverImageList.toString()); // 이건 파일 이름 크기 확장자 등 정보 다나와서 보기 힘들다
//		
//		// 파일 리스트 확인
 ...
 * 
 * 설정 방법 
 * 1) XXX Application.java 파일에 @EnableScheduling 어노테이션 추가
