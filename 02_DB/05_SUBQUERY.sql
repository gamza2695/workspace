...
-- 1) 전 직원의 평균 급여 조회하기
SELECT AVG(SALARY)FROM EMPLOYEE; -- 3047763

-- 2) 직원들중 급여가 3047663원 이상인 사원들의 사번, 이름, 직급코드, 급여 조회
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY > = 3047763;
...
-- 2) 부서별 최고 급여를 받는 직원의 이름, 직급, 부서, 급여를 부서 순으로 정렬하여 조회
...
-- 1) 사수에 해당하는 사원 번호 조회
SELECT DISTINCT MANAGER_ID -- DISTINCT : 중복 값 한번만
FROM EMPLOYEE
...
-- 방법 1 : SET OPRATION(집합 연산자) 중 UNION 사용ㅇ
-- 3번
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, '사수' "구분"
FROM EMPLOYEE
NATURAL JOIN JOB
...
-- 방법2. 선택 함수(CASE) 이용 방법

SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, 
	CASE 
		-- 현재 행의 EMP_ID가 서브쿼리 결과 중 하나인 경우
		WHEN EMP_ID IN(SELECT DISTINCT MANAGER_ID -- DISTINCT : 중복 값 한번만
						FROM EMPLOYEE
						WHERE MANAGER_ID IS NOT NULL)
		THEN '사수'
		ELSE '사원'
...
AND SALARY > ANY (SELECT SALARY
				FROM EMPLOYEE
				JOIN JOB USING(JOB_CODE)
				WHERE JOB_NAME = '과장'); -- 3760000,2200000(최소), 2500000
...
-- WHERE DEPT_TITLE = '기술지원부' AND JOB_NAME = '대리';
...
WHERE HIRE_DATE = --(특정 부서 입사일이 가장 빠른 사람 SELECT)
		...
		-- 소속없음(하동운,이오리) : NULL은 비교연산 불가 => NVL 사용
		AND ENT_YN != 'Y'
...
--		(SELECT MIN(HIRE_DATE) 
--		FROM EMPLOYEE SUB
--		WHERE (SUB.DEPT_CODE = MAIN.DEPT_CODE -- 소속없음 : NULL이돼서 비교연산 불가
...
SELECT 사번, 이름 FROM 
	...
	FROM EMPLOYEE
	LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID));
--> 서브쿼리의 결과 RESULT SET이 다른 SELECT문의 테이블 역할(인라인:다른 셀렉트안에서 뷰:볼수만 있다)
...
-- 3) 1번에 ROWNUM 컬럼 추가 --> 3번할때 ROWNUM 섞임
--> 해결 방법 : 인라인 뷰 사용
...
-- * ROWNUM 사용 시 주의사항 *
-- ROWNUM을 WHERE절에 사용할 때 항상 범위에 1부터 연속적인 범위가 포함되어야 한다
SELECT ROWNUM, EMP_NAME, SALARY
FROM (SELECT EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY DESC
)
-- WHERE ROWNUM =1 ; -- 1행만 조회 가능
-- WHERE ROWNUM = 2; -- 2행만 조회 --> 실패
WHERE ROWNUM BETWEEN 1 AND 10; -- 1부터 포함!!
 ...
 * EX1) ORDER BY 절에서는 SELECT절에서 해석된 별칭 사용 가능
 * EX2) 인라인 뷰에서 지정된 별칭을 메인쿼리에서도 똑같이 사용해야 된다
 * 
 * 3. 여러 테이블을 이용한 SELECT 진행 시 
 *    컬럼명이 겹치는 경우 이를 해결하는 방법
 * EX1) 셀프 조인
 * EX2) 상관 쿼리
 * EX3) 다른 테이블이여도 컬럼명이 같을 때 
 * 
...
TO_CHAR(SYSDATE,'YYYY') - ('19' || SUBSTR(EMP_NO,1,2)) 나이, SALARY*12*(1+NVL(BONUS,0)) 보너스포함연봉
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
WHERE TO_CHAR(SYSDATE,'YYYY') - ('19' || SUBSTR(EMP_NO,1,2)) 
< ALL(SELECT TO_CHAR(SYSDATE,'YYYY') - ('19' || SUBSTR(EMP_NO,1,2))
FROM EMPLOYEE)
ORDER BY 나이 DESC;
