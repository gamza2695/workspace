...
-- 계정 생성
CREATE USER a230830_jyj IDENTIFIED BY "4453";
--> 생성 후 DB 접속 방법 추가
-- ORA-01045: 사용자 A230830_JYJ는 CREATE SESSION 권한을 가지고있지 않음; 
-- 로그온이 거절되었습니다
--> 접속 권한 없음

/* 접속 권한 부여(GRANT)*/
GRANT CREATE SESSION TO a230830_jyj; 
-- 접속됨
...
----------- [관리자 계정] ----------- 
-- 테이블 생성 권한, TABELSPACE 할당
GRANT CREATE TABLE TO a230830_jyj;

ALTER USER a230830_jyj DEFAULT TABLESPACE "A230724"
...
-- SYS 계정으로 ROLE 확인하기
SELECT * FROM ROLE_SYS_PRIVS -- 생성권한보기!
WHERE ROLE = 'RESOURCE'; -- 안보임 관리자 권한으로 들어가야 보임

-- 테스트 사용자 계정에 접속, 기본 객체 8개, VIEW 생성권한 부여하기
GRANT CONNECT, RESOURCE, CREATE VIEW TO a230830_jyj;
...
REVOKE CONNECT FROM a230830_jyj;
