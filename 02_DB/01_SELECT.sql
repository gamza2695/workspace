...
-- DISTINCT (별개의, 따로따로)
--> 조회 시 지정된 컬럼에 중복되는 값을 한번만 표시할 때 사용
--> 중복 제거
...
--		FALSE면 미포함
-- 3) 추려진 결과행 중 알맞은 컬럼만 선택 -> 최종 RESULT SET 생성
...
-- 사번, 이름, 입사일, 급여 조회
-- 컬럼명 BETWEEN a AND b : a 이상 b 이하면 TRUE
...
-- 사번, 이름, 입사일, 급여 조회
-- 컬럼명 NOT BETWEEN a AND b : a 이상 b 이하면 FALSE = a 미만, b 초과면 TRUE 
...
-- 컬럼명 NOT IN (값1, 값2, 값3,...)
--> 컬럼의 값이 () 내부의 값과 하나라도 일치하면 FALSE = 하나도 일치 안하면 TRUE
SELECT EMP_NAME 이름, DEPT_CODE 부서코드, SALARY 급여
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서
-- EMAIL의 아이디 중 '_' 앞 쪽 글자의 수가 3글자이 사원의 
-- 사원의 사번, 이름, 이메일 조회
...
--> 사용자(개발자)가 직접 SQL에 날짜를 입력할 땐 날짜 형식의 문자열을 작성한다! (2023/08/22, 2023-08-22, 20230822)
...
FROM EMPLOYEE
ORDER BY SALARY ASC; -- ASC(ascendting) : 오름차순
...
FROM EMPLOYEE
WHERE DEPT_CODE IN('D5', 'D6', 'D9')
ORDER BY DEPT_CODE; --ASC가 기본값(생략 가능)


-- EMPLOYEE 테이블에서
-- 급여가 300만 이상, 600만 미만 사원
...
/*1*/FROM EMPLOYEE
/*2*/WHERE SALARY*12 >=35000000 -- 별칭 수식 적용 불가
/*4*/ORDER BY 연봉; 
...
/* SELECT 절에 작성되지 않은 컬럼으로 정렬 기준 작성 가능*/
-- 이름, 급여를 사번 내림차순으로 조회

SELECT EMP_NAME 이름, SALARY 급여--, EMP_ID 사번
FROM EMPLOYEE
