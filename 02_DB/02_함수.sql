...
-- 단일 행 함수 : N개의 행의 컬럼값을 전달하여 N개의 결과 반환

-- 그룹 함수 : N개의 행의 컬럼값을 전달하여 1개의 결과 반환
--			(그룹의 수가 늘어나면 그룹의 수 만큼 결과를 반환)

-- 함수는 SELECT절, WHERE절, ORDER BY 절
-- 		GROUP BY절, HAVING절에서 사용 가능 -- 어디서든지!
...
-- INSTR(문자열 | 컬럼명, '찾을 문자열' [, 찾을 시작 위치 [, 순번]]) -- 대괄호는 필수X
...
SELECT REPLACE(NATIONAL_NAME,'국', '국국')
FROM NATIONAL;
...
-- 소수점 위치 지정 X : 소수점 첫째자리에서 반올림 -> 정수 표현
-- 소수점 위치 지정 O 
...
SELECT 123.456,
	...
	ROUND(123.456, -2) -- 100 십의 자리에서 반올림 백의 자리까지 표현
FROM DUAL;
...
SELECT ADD_MONTHS(SYSDATE,-1) "한달 전", SYSDATE, ADD_MONTHS(SYSDATE,1)  "한달 후" FROM DUAL;
...
-- EXTRACT(YEAR | MONTH | DAY FROM 날짜) : 지정된 날짜의 년 | 월 | 일을 추출하여 정수로 반환
...
SELECT EMP_ID 사번, EMP_NAME 이름, TO_CHAR(SALARY*12,'L99,999,999') 연봉
FROM EMPLOYEE;
...
-- 50 미만
SELECT TO_DATE('49-11-25', 'YY-MM-DD') "YY", -- 2049-11-25 00:00:00.000
		TO_DATE('49-11-25', 'RR-MM-DD') "RR" -- 2049-11-25 00:00:00.000
FROM DUAL; 

-- 50 이상
SELECT TO_DATE('50-11-25', 'YY-MM-DD') "YY", -- 2050-11-25 00:00:00.000
		TO_DATE('50-11-25', 'RR-MM-DD') "RR" -- 1950-11-25 00:00:00.000
FROM DUAL; 
...
-- SELECT NULL/10 FROM DUAL; --> NULL 산술연산 결과는 항상 NULL
...
SELECT EMP_ID 사번, EMP_NAME 이름, NVL2(PHONE, SUBSTR(PHONE,1,3)||'********','없음') 전화번호
FROM EMPLOYEE;

SELECT EMP_ID, EMP_NAME, 
-- RPAD("값", "총 문자길이", "채움문자")
   NVL2(PHONE, RPAD(SUBSTR(PHONE,1,3), LENGTH(PHONE), '*') , '없음')
FROM EMPLOYEE;
...
SELECT EMP_NAME 이름, EMP_NO 주민등록번호, DECODE(SUBSTR(EMP_NO,8,1),1,'남자',2,'여자') 성별
FROM EMPLOYEE;
...
-- : N개의 행의 컬럼값을 전달하여 1개의 결과 반환 (그룹의 수가 늘어나면 그룹의 수 만큼 결과를 반환)
...
/*1*/FROM EMPLOYEE				-- 1) EMPLOYEE 테이믈에서
...
-- COUNT(컬럼명) : 지정된 컬럼값이 NULL이 아닌 행의 개수를 반환(NULL인 행 미포함) 
...
SELECT COUNT(*) FROM EMPLOYEE 
WHERE PHONE IS NOT NULL; -- PHONE 컬럼이 NULL아닌 행만 카운트
...
SELECT COUNT(DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남자', NULL)) 남자, -- NULL을 뺀 행 수 : 남자
