...
-- 생성한 뷰 V_EMPLOYEE를 이용해서 SELECT 
SELECT * FROM V_EMPLOYEE;

-- V_EMPLOYEE에서 대리 직급 사원을 이름 오름차순으로 조회
SELECT * FROM V_EMPLOYEE
WHERE 직급명 = '대리'
ORDER BY 이름;
...
--> 옵션 추가 후 다시 실행 CREATE OR REPLACE VIEW V_EMPLOYEE
...
--> NULL 삽입 == 무결성 약하게(위배) 만드는 원인

ROLLBACK;

-- WITH READ ONLY 옵션 추가 후 다시 INSERT 
...
NOMINVALUE 		-- 최소값 없음 / CYCLE일때 되돌아감
NOCYCLE	 		-- 최대값까지 증가 후 반복 X 
-- CACHE 			-- 미리 발생시킬 숫자를 만들어둠
...
-- MAXVALUE은 사례로 될 수 없습니다 -> MAXVALUE 초과
SELECT * FROM TB_TEST;
...
SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL; -- 200까지 가능
...
-- 시퀀스 START WITH를 다시 지정하고 싶으면 지우고 다시 생성해야만 한다!
...
-- [A230724_test 계정 접속] : test 계정으로 변경
SELECT COUNT(*) FROM TB_INDEX_TEST; -- 데이터 100만개

-- 인덱스 사용 X

SELECT * FROM TB_INDEX_TEST
WHERE TEST_ID = 'TEST500000'; -- 18 ~ 22ms

-- 인덱스 사용 O
SELECT * FROM TB_INDEX_TEST
WHERE TEST_NO = 500000; -- 0 ~ 3ms 빠르다!
