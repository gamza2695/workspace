...
-- RIGHT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- RIGHT JOIN (24행,마케팅,국내영업,해외영업3)
...
JOIN JOB USING (JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE EMP_NO LIKE '7%' -- SUBSTR(EMP_NO,1,1)='7' 
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE);
-- WHERE DEPT_CODE IS NOT NULL; INNER JOIN에서는 NULL 제외!!
...
SELECT EMP.EMP_NAME 사원명, EMP.DEPT_CODE 부서코드, EMP2.EMP_NAME 동료이름
FROM EMPLOYEE EMP
RIGHT JOIN EMPLOYEE EMP2 ON (EMP.DEPT_CODE = EMP2.DEPT_CODE)
WHERE EMP.EMP_NAME <> EMP2.EMP_NAME -- 동명이인 생각해서 EMP_ID로,,
ORDER BY 사원명; -- = ORDER BY 1;
