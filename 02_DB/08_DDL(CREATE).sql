...
/* 자료형
    NUMBER : 숫자형(정수, 실수) 38자리
    CHAR(크기) : 고정길이 문자형 (2000BYTE)  // 자바char : 문자1개 다름 구분!
        -> ex) CHAR(10) 컬럼에 'ABC' 3BYTE 문자열만 저장해도 10BYTE 저장공간을 모두 사용. 
        
    VARCHAR2(크기) : 가변길이 문자형 (4000 BYTE) // 속도는 CHAR가 빠르고 VARCHAR가 효율적
...
-- 만든 테이블 확인
SELECT * FROM MEMBER;
SELECT * FROM USER_TABLES; -- 딕셔너리 뷰 중 USER_TABLES 
...
-- 자바에서 NULL : 참조X, DB에서 NULL: 값없음(비어있음)
...
-- ORA-00001: 무결성 제약 조건(A230724_JYJ.SYS_C0013065)에 위배됩니다
-- SYS_C0013065 : 제약조 조건 이름 미설정 시 자동으로 지정되는 이름
...
-- 테이블에 대한 식별자(IDENTIFIER) 역할을 함 --> 학번, 사번, 주민등록번호, 시리얼넘버, 번호표, 게시글 번호...
...
CREATE TABLE USER_USED_PK(
    ...
    -- USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY, -- 컬럼 레벨, 이름 지정 O 권장!
...
--> 기본키 중복으로 오류 - ORA-00001: 무결성 제약 조건(A230724_JYJ.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류 - ORA-01400: NULL을 ("A230724_JYJ"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(A230724_JYJ.PK_USERNO_USERID)에 위배됩니다
...
-- NULL을 ("A230724_JYJ"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
-- 참조(REFERENCES)된 다른 테이블의 컬럼이 제공하는 값만 사용할 수 있음
	...
	--> 두 테이블이 같은 컬럼 값을 지님 ==> JOIN에 활용

-- FOREIGN KEY제약조건에 의해서 테이블간의 관계(RELATIONSHIP)가 형성됨 : RDBMS
...
CREATE TABLE USER_GRADE(
  GRADE_CODE NUMBER PRIMARY KEY, -- 얘만 참조당할 수 있다!
  GRADE_NAME VARCHAR2(30) NOT NULL
);
INSERT INTO USER_GRADE VALUES (10, '일반회원');
INSERT INTO USER_GRADE VALUES (20, '우수회원');
INSERT INTO USER_GRADE VALUES (30, '특별회원');
--> 참조하는 곳에서는 10,20,30,NULL만 쓸 수 있는 것
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  -- 1) GRADE_CODE NUMBER REFERENCES USER_GRADE(GRADE_CODE) -- 컬럼 레벨
  -- 2) GRADE_CODE NUMBER // CONSTRAINT GRADE_CODE_FK1// REFERENCES USER_GRADE(GRADE_CODE) -- 이름 지정
  GRADE_CODE NUMBER,
  
  -- 테이블 레벨
  -- 3) FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE(GRADE_CODE) -- 이름 지정 X
  -- 4)
...
-- ORA-02291: 무결성 제약조건(A230724_JYJ.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
...
--> ORA-02292: 무결성 제약조건(A230724_JYJ.GRADE_CODE_FK1)이 위배되었습니다
--  - 자식 레코드가 발견되었습니다
...
-- 자식 테이블 조회
SELECT * FROM USER_USED_FK2; --> 홍길동, 이순신의 GRADE_CODE가 10 -> NULL 변화 확인
...
-- ORA-02290: 체크 제약조건(A230724_JYJ.GENDER_CHECK)이 위배되었습니다


SELECT * FROM USER_USED_CHECK;

-- INSERT뿐 아니라 UPDATE에도 적용
...
WHERE USER_NO =1; -- ORA-02290: 체크 제약조건(A230724_JYJ.GENDER_CHECK)이 위배되었습니다
...
CREATE TABLE USER_TEST(
	...
	--	PNO CHAR(14) CONSTRAINT NN_PNO NOT NULL CONSTRAINT UK_PNO UNIQUE(PNO),가능
	GENDER VARCHAR2(10),
	-- GENDER CHAR(3) CONSTRAINT CK_GENDER CHECK(GENDER IN('남','여')),
	-- 참고) GENDER NVARCHAR2(1), : 유니코드 글자 수 1개
	PHONE VARCHAR2(20),
	ADDRESS VARCHAR2(50),
...
INSERT INTO USER_TEST
VALUES(10, 'user10', '011234', '123456-1234567', '남', '010-0000-0001', '서울시 중구', 'N');

-- 기본키 제한 확인
INSERT INTO USER_TEST
VALUES(10, 'user20', '011235', '123456-1234568', '남', '010-0000-0002', '서울시 중구', 'Y');

-- 중복 확인
INSERT INTO USER_TEST
VALUES(20, 'user10', '011234', '123456-1234567', '남', '010-0000-0001', '서울시 중구', 'N');

-- NULL값 제한 확인
INSERT INTO USER_TEST
VALUES(30, 'user30', NULL , NULL , '여', '010-0000-0003', '서울시 중구', NULL);

-- 체크 제한 확인
INSERT INTO USER_TEST
VALUES(10, 'user40', '011238', '123456-1234567', '남자', '010-0000-0005', '서울시 중구', 'K');

DELETE FROM USER_TEST WHERE USER_NO = 1;
...
-- 컬럼명, 데이터 타입, 값이 복사되고, 제약조건은 NOT NULL 만 복사됨 - EMPLOYEE와 EMPLOYEE_COPY 비교!
...
AS SELECT EMP_NAME, NVL(DEPT_TITLE,'소속없음') DEPT_TITLE, JOB_NAME FROM EMPLOYEE
...
/* NOT NULL만 MODIFY로 작성*/ -- NOT NULL 은 체크제약조건의 일부! 많이 써서 하나로 보게 됨
...
FOREIGN KEY (DEPT_CODE) REFERENCES DEPARTMENT;
							--(DEPT_ID); 참조 테이블 컬럼명 생략가능 -> 생략하면 PK와 연결
...
CONSTRAINT SAL_GRADE_FK -- 제약 조건 이름 지정
FOREIGN KEY(SAL_LEVEL) REFERENCES SAL_GRADE;
						-- FK 제약 조건 추가(EMPLOYEE.SAL_LEVEL 컬럼이 SAL_GRADE의 PK 참조)
