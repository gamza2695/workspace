...
// DAO(Database Access Object) : DB와 연결하여 SQL 수행 후 결과 반환받는 객체
public class MemberDAO {
	
	// JDBC 객체를 저장할 변수(필드) 선언
	private Statement stmt = null; // SQL 수행 후 결과 반환받는 역할
	private PreparedStatement pstmt = null; // Statement + "?"(placeholder)로 SQL 쉽게 작성
	...
	public List<Member> selectMemberList(Connection conn) {
		...
		try {
			...
			// 5) SQL(SELECT) 수행 후(executeQuery()) 후
			// 결과(ResultSet) 반환 받기
			rs = pstmt.executeQuery();
