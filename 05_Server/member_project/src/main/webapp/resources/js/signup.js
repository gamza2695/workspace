...
/* form태그 name 속성 값이 있으면 document.name 속성값으로 얻어올 수 있음 */
document.signupForm.addEventListener("submit", e =>{

  if(inputCheck("id")) { e.preventDefault(); return;}
  if(inputCheck("pw")) { e.preventDefault(); return;}
  if(inputCheck("name")) { e.preventDefault(); return;}
