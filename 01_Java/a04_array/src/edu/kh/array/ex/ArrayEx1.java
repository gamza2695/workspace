...
public class ArrayEx1 {
	...
	public void ex1() {
		...
		// 배열 선언
		int[] arr; 
		// int 자료형 변수를 묶어서 다르는 배열 arr을 선언
		// int[]은 참조형
		...
		// 배열 대입 (== 할당)
		arr = new int[3];
		// new 연산자 : Heap 영역에 새로운 메모리를 할당(할당 : 메모리 공간을 차지한다)
		...
		// -> arr 호출 시 arr에 저장된 주소를 참조해 Heap 영역에 할당된 배열을 찾아감
		//    --> 이래서 int[]을 "참조형"이라고 한다.
		
	
		// * 배열 다루기 *
		System.out.println("arr : " + arr); // arr : [I@6f2b958e -> 주소값 출력
	...
	public void ex2() {
		...
		//int[] numbers;
		//numbers = new int[4]; // new 연산자 : Heap 영역에 메모리를 새롭게 할당
	...
	// 3명의 키를 입력 받아 입력받은 키 + 평균 키 출력(배열 이용)
	public void ex3() {
		Scanner sc = new Scanner(System.in);
		
		// double형 배열을 참조할 변수 heightArray 선언(주소 저장) 후 
		// Heap 영역에 double 3칸짜리 배열을 할당하여 그 주소를 heightArray에 저장
...
//		menuArray[4] = "닭갈비";
	...
	public void ex5() {
		...
		// 배열 2개를 생성 후 같은 인덱스 번호에 메뉴명, 가격 작성
		...
		while(true) {
			...
			if(input == 9) { //만약에 9를 입력한 경우 반복문을 멈춰라
				break;
			}
	...
	public void ex6() {
		...
		int[] scores = new int[num]; // 입력 받은 수 만큼의 길이를 갖는 배열 할당
		int sum = 0;
		
						// ==num.length
		for(int i = 0; i<scores.length; i++) {
			System.out.printf((i+1)+"번 점수 입력 : ");
			scores[i] = sc.nextInt(); // 입력 받은 정수를 배열 요소로 저장
	...
	public void ex7() {
		...
		while(true) {
			...
			if(check<0 || check>=num.length) {
				System.out.println("존재하지 않는 인덱스 번호 입니다\n");
				continue;
			}
			
			System.out.println(check + "번 인덱스 : " + num[check] +"\n");
	...
	// 얕은 복사 : 참조하는 주소만을 복사하여 서로 다른 참조 변수가 하나의 배열을 참조하게 만드는 것
	public void ex8() {
	...
	// 깊은 복사
	public void ex9() {
		//  : 원본과 같은 참조형 변수와 원본과 같거나 더 큰 배열을 만들어 
		...
		// 3. arr3 = Arrays.copyOf(arr, arr.length); 잘 안씀!
		
		// 4. arr4 = arr.clone(); 오래된 함수지만 위험한 함수!
