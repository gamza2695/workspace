...
public class ArrayEx2 {
	...
	public void ex2() {
		...
		// 자바는 실제로 String 문자열을 char배열 형식으로 저장
		...
		for(int row=0; row < arr.length; row++) {
			for(int col = 0; col< arr[row].length ; col++ ) {
				...
				if(arr[row][col] == input) {
					System.out.printf("%c는 %d행 %d열에 있습니다.", input, row, col);
					flag = false;
				}
			}
		}
		...
		if(flag) { // flag가 true이면 if 실행, false이면 실행x
	...
	public void ex3() {
		...
		int[][] arr = {{30, 34, 23, 1}, {32, 43, 65, 11}, {103, 33, 124, 65}};
		...
		for(int row = 0; row<arr.length; row++) {
   		...
   		// 향상된 for문으로 쓰기 : 모든 배열 요소를 접근하는 용도
    	// 장점 : 작성 간단 / 단점 : 시작, 끝, 증감, 인덱스 지정 불가능
        
        // for(배열 요소의 자료형으로 된 변수 선언 : 배열명)
        // -> 오른쪽에 작성한 배열의 요소를 순서대로 하나씩 꺼내와 대입
//		for(int[] row : arr) {
//			for(int[] col : row) {
//				sum+=col;
//			}
//		}
	...
	public void ex4() {
		...
		// 최대값, 최소값 == 비교 == 비교값이 존재해야함
		// -> 이럴 때는 비교군의 첫번째 값을 이용
		int max = arr[0][0];
		int min = arr[0][0];
		...
		for(int row = 0 ; row<arr.length; row++) {
			for(int col = 0; col < arr[row].length; col++) {
				...
				if(arr[row][col]>max) { // 최대
					max = arr[row][col];
				}
				if(arr[row][col]<min) { // 최소
					min = arr[row][col];
				}
			}
			System.out.println();
