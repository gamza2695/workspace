...
public class InterruptTest {
	 ...
	 * 
	 * void interrupt()
	 * - 현재 스레드의 interrupted 필드값을 true로 변경(원래 start하면 interrupted 값은 false)
	 *   interrupted 필드 == false -> 작업 진행
	 *   interrupted 필드 == true -> 작업 멈춤
	 *   
	 *   단, Thread.sleep()에 의해서 일시 정지된 스레드에
	 *   intterupt()를 호출하게 되면
	 *   스레드의 interrupted 필드값이 true로 변하지 않고 작업 종료 안됨
	 *   InterruptedException 발생한다
	 * 
	...
	public void test() {
		...
		System.out.println("<엔터 입력 시 종료>"); // 0.5초 전에 실행되므로 제일 먼저 나올 것
