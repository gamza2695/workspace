...
public class CarRun {
	public static void main(String[] args) {
		...
		// 1. 다형성과 객체 배열
		Car[] carArr = new Car[3]; // Car(부모) 객체 참조 변수 3개의 묶음
		...
		for (int i = 0; i < 6; i++) { // ctrl + shift + f : 이클립스 자동정렬 설정,, 정확도 높지 않음 간단할때 사용하기
			...
			if (car1 != null) { // 인덱스 정상 범위
				...
				// 참조변수 instanceof 클래스명 : 다운캐스팅에 많이 쓰임 (미리 검사!)
				// -> 참조변수가 참조하는 객체가 클래스명 타입을 가지고 있는가?
				// 클래스명 객체인가?
				// 클래스명 타입과 상속 관계인가?
				// 클래스명 타입으로 형변환이 가능한가?
				if (car1 instanceof Porsche) { // Porsche인 경우 뚜껑 열림?
					System.out.println("뚜껑 열림? : " + ((Porsche) car1).isOpenTheDoor());
				}

				if (car1 instanceof Spark) { // Spark인 경우 할인율
					System.out.println("할인율 : " + ((Spark) car1).getDiscountRate());
				}

				// ClassCastException : 올바르지 않은 자료형으로 형변환 시 발생
