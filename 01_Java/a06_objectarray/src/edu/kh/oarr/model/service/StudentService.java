...
public class StudentService {
	...
	// 필드
		// 학생 객체 참조 변수 10개 묶음 Student[]을 할당하고 그 주소를 studentArr에 대입
	...
	// - 기본 생성자(: 매개변수가 없는 생성자)
	// 학생 객체 배열에 들어갈 샘플 학생 데이터를 3개 추가(0,1,2)
	...
	public boolean addStudent(int grade, int ban, int number, String name) {
		...
		// 1. studentArr의 요소 중 참조하는 게 없는 배열요소를 찾아
		// 그 중 index 번호가 가장 낮은 요소에 학생 객체 주소를 대입
		// true 반환
		for(int i = 0; i<studentArr.length;i++) {
			if(studentArr[i]==null) { // 참조하는 게 없을 경우 학생 객체를 생성하여 주소 대입
	...
	public Student selectIndex(int index) {
		...
		// index번째 요소에 학생 객체를 참조하는 주소가 있는 경우 그 주소 반환
	...
	public Student[] selectName(String name) {
		...
		for(Student s : studentArr) {
			...
			// NullPointerException (null을 참조해서 필드, 메서드 접근 시 발생) 방지하기
			if(s==null) {
				break;
			}
		...
		int index = 0; // 검색된 학생 객체의 주소를 arr 배열의 몇 번 인덱스 대입해야 되는지를 제어
		
		for(Student s :studentArr) {
			if(s==null) { // NullPointerException 방지
				break;
			}
			if(s.getName().equals(name)) { // 이름이 일치하는 같은 학생일 경우 
				arr[index] = s;
				index++;
			}
		}
