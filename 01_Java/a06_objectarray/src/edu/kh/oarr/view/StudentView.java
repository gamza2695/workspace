...
public class StudentView {
	...
	private void selectAll() {
		...
		for( Student s: arr) { // Student[] 의 배열요소 하나는 Student
	...
	private void selectIndex() {
		...
		// StudentService로 index를 전달하여 해당하는 index에 
		// 학생 객체가 있으면 그 객체의 주소를 반환
	...
	private void selectScore() {
		...
		String s = service.selectScore(index);
	...
	public void updateStudent() {
		...
		// 인덱스 번호를 입력 받고 해당 학생이 존재하면 
		// 국어, 영어, 수학 점수를 입력 받아 수정 후
		// "수정되었습니다" 출력
		...
		Student s = service.updateStudent(index);
		// 이미 존재하는 service의 selectIndex(index) 메서드 이용
		
		// 2. 존재하지 않는 학생인 경우 "해당 인덱스에 학생 정보가 존재하지 않습니다" 출력 후 return
		
		// 3. 2번이 실행되지 않은 경우(== 존재 하는 경우)
		// 국어, 영어, 수학 점수를 입력 받아 (반환받은 학생 객체 주소, 국어, 영어, 수학) 4개를
