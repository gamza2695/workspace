...
public class SwitchEx {
	...
	public void ex2() {
		...
		// (int)Math.random() * 5 + 1 : 1.0 <= x * 5 + 1 < 6.0
	...
	public void ex3() {
		...
		case "떡뽁이" 	: price = 4000; break;
		default 		: price = -1; // -1은 잘못 입력되었다는 신호
		}
	...
	public void ex4() {
		...
		int result = 0; // 초기화했기 때문에 switch문에서 값이 
						// 대입되지 않아도 이후 result 출력 시 문제 발생X
						// 아무 숫자나 초기화해도 ok
		...
		case "/" : result = num1 / num2; break;
		case "%" : result = num1 % num2; break;
		default  : flag = true; // 연산자 잘못 입력 -> flag를 true로 변경
		// default : result = -1; // 결과 3 - 4 = -1!! 
		}
		
		if(!flag) { //정상 입력 == flag가 false = !flag
	...
	public void ex5() {
		...
		//* break가 없는 경우 *
		// - break를 만나거나 switch가 끝날 때 가지 다음 case를 연달아 실행
