...
public class SetEx {
	...
	public void ex1() {
		...
		// Set<String> set = new LinkedHashSet<String>(); // 순서유지
		...
		System.out.println(set); // HashSet은 순서 유지X
		...
		// 2. int set.size() : 저장된 객체 수 반환
		System.out.println("size : " + set.size());
		
		// 3. boolean remove(E e)
		// e와 똑같은 요소가 set에 있으면 지우고 true 반환 없으면 false 반환
		if(set.remove("직방")) {
		...
		set.clear();
		System.out.println("clear 후 seze" + set.size());
		
		
		// 6. boolean set.isEmpty() :
		// set에 저장도니 객체가 없다면(비어있다면) true, 아님 false
		// set.size()==0 과 같은 뜻
		System.out.println("set이" + (set.isEmpty()? "비어있음":"비어지있지 않음"));
	...
	public void ex2() {
		...
		// Set에 저장되어 있는 갑을 꺼내서 쓰는 방법
		
		// Iteratble 특징을 띈다 : 나열할 수 있는, 순서대로 하나씩 꺼낼 수 있다
		// -> 자바r가 String 증가
	...
	public void lottoVersion2() {
		...
		// 금액을 입력 받아서(천원 단위)
		// 3천원이면 3회, 5천원이면 5회치 로또 번호를 자동으로 생성해서 출력
