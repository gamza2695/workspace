...
public class ListEx {
	...
	public void ex1() {
		...
		// 다형성 적용
//		List list = new ArrayList(); // 10칸 짜리 생성<기본>
		...
		// Auto Boxing
		list.add(123); // int -> 오토박싱 -> Integer
		list.add(3.14); // double -> 오토박싱 -> Double
		...
		// list의 크기 3을 초과해서 추가해도 자동으로 크기가 늘어나서 에러 발생 x
		list.add("크기 3 초과");
		System.out.println(list); 
		...
		for(int i=0; i<list.size();i++) {
			...
			if(list.get(i) instanceof Double) { // Double인 경우
				Double d = (Double)list.get(i); // 오토언박싱 안하고 다운캐스팅만
