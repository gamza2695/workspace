...
public class StudentView {
	...
	// 이름 클릭하고 alt + shift + j => 메서드/클래스 설명문 주석
	/**
	 * 메뉴 출력용 view 메서드
	 */
	public void displayMenu() {
		...
		do {
			...
			try { // 예외가 발생할 수 있는 코드 작성 부분
				...
				// Scanner 입력버퍼 문제발생
				// Scanner 입력 버퍼에 남아있는 잘못 입력된 값을 제거하는 코드 작성
				// String sc.nextLine(); : 입력 버퍼에서 한줄(\n)까지 읽어오는 기능
				sc.nextLine();
			}
	...
	private void selectAllStudent() {
		...
		for(int i = 0 ; i<studentList.size(); i++) {
			...
			// Student타입 String 안맞지만 studentList.get(i).toString()으로 출력된다
	...
	private void selectOne() {
		...
		if(std==null) {
			...
			System.out.println(std); // std.toString() .toString() 자동으로 붙여줌
		}
	...
	private void selectGender() {
		...
		char gender = sc.next().toUpperCase().charAt(0);
					// String / 대문자로 변경 / 0번째 문자 하나 -> 무조건 대문자
		...
		// if(searchList == null) -> 아예 반환되는 값, 주소가 없다는 의미
	...
	private void duplicateValidation() {
		...
		if(result) {
			...
			System.out.println("같은 학생 없음 XXXXX");
		}
